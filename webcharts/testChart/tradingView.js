...
const config_login = {
	...
	database: config.database_login
};
const config_bong = {
	user: config.user,
	password: config.password,
	server: config.server,
	database: config.database_bong
};
...
let serchFutic = () => {
	return new Promise((resolve, reject) => {
		...
		let mm = today.getMonth() + 1 > 9 ? today.getMonth() + 1 : "0" + (today.getMonth() + 1); //January is 0!
		let yyyy = today.getFullYear();
		today = yyyy + mm + dd + ' 14:00:00';
		let query = "SELECT * FROM tbl_futic WHERE 일자 >= '" + today + "' ORDER BY 일자 ASC;";

		let decodeQuery = decodeURIComponent(query);
		new mssql.ConnectionPool(config_bong).connect().then(pool => {
			return pool.request().query(decodeQuery);
		}).then(result => {
			mssql.close();
			resolve(result);
		}).catch(err => {
...
function fixedDecimalPlaces(num) {
	...
	else if (num2.toString().length == 4) {
		num2 = num2 + "0";
	}
...
RequestProcessor.prototype._sendSymbolHistory = function (symbol, startDateTimestamp, endDateTimestamp, resolution, response) {
	...
	function logForData(data, key, isCached) {
		...
		var toCacheTime = data && data.t ? data.t[data.t.length] : null;
	...
	serchFutic().then(result => {
      ...
      for (var j = result.recordset.length - 1; j >= 12; j--){
        for(var k = 1; k < 13; k++){
          s12HighSma += parseFloat(result.recordset[j - k]['시가']);
          s12LowSma += parseFloat(result.recordset[j - k]['시가']);
        }
        // S라인 12일 상선
        s12High = (s12HighSma / 12) * 1.0988;
        s12High = (Math.ceil(s12HighSma));
        s12HighSma = 0;

        // S라인 12일 하선
        s12Low = (s12LowSma / 12) * 1.0988;
        s12Low = (Math.ceil(s12LowSma));
        s12LowSma = 0;

        s12HighSum.push(s12High);
        s12LowSum.push(s12Low);
      }
      // s12HighSum = s12HighSum.reverse();
      // s12LowSum = s12LowSum.reverse();

			value.s = "ok";

			var i = result.recordset.length;
			result.recordset.forEach(element => {
				var elementDate = [(element['일자']).substr(0, 4), (element['일자']).substr(4, 2), (element['일자']).substr(6, 2)];
				var someDate = new Date(elementDate.join('-') + element['일자'].substr(8, 16));
				someDate.setDate(someDate.getDate() - i);
				someDate = someDate.getTime() / 1000;

				var o = []; // open, s-line(12) high, s-line(12) low
				o[0] = parseFloat(element['시가']).toFixed(2); // open
				o[1] = s12HighSum[i].toFixed(2) * 100; // s-line(12) high
				o[2] = s12LowSum[i].toFixed(2) * 100; // s-line(12) low


				var h = []; // high, s-line(30) high, s-line(30) low
				h[0] = parseFloat(element['시가']).toFixed(2) * 100; // high
				h[1] = parseFloat(element['시가']).toFixed(2) * 100; // s-line(30) high
				h[2] = parseFloat(element['시가']).toFixed(2) * 100; // s-line(30) low

				var l = []; // low, c1 high
				l[0] = parseFloat(element['저가']).toFixed(2); // low
				l[1] = fixedDecimalPlaces(Math.abs(Math.round(parseFloat(element['c1h']).toFixed(2) * 100))); // c1 high

				var c = []; // close, c1 low
				c[0] = parseFloat(element['종가']).toFixed(2); // close
				c[1] = fixedDecimalPlaces(Math.abs(Math.round(parseFloat(element['c1l']).toFixed(2) * 100))); // c1 low

				var test = l[1].toString() + "/" + l[2].toString();
				var data = [];
				var dataStr = "";
				for (var j = 0; j < test.length; j++) {
					// data[j] = new Array();
					// for (var k = 0; k < l[j].toString().length; k++){
						// data[j][k] = (l[j].toString().charCodeAt(k));
						data.push(test.charCodeAt(j));
						dataStr += "" + test.charCodeAt(j);
					// }
				}

				var wwww = "";
				var zxcv = parseFloat(element['c1h']).toFixed(2).split(".");
				if (zxcv[0] == '0') {
					wwww = "6";
				}
				else if (zxcv[0] != '0' && zxcv[0].length == 1) {
					wwww = "4";
				}
				else if (zxcv[0].length == 2) {
					wwww = "3";
				}
				else {
					wwww = "2";
				...
				var eeee = "";
				var vcxz = parseFloat(element['c1l']).toFixed(2).split(".");
				if (vcxz[0] == '0') {
					eeee = "6";
				}
				else if (vcxz[0] != '0' && vcxz[0].length == 1) {
					eeee = "4";
				}
				else if (vcxz[0].length == 2) {
					eeee = "3";
				}
				else {
					eeee = "2";
				...
				var dddd = "";
				var xxxx = "";
				if (parseInt(l[1]) > 0 && parseInt(l[2]) > 0) {
					dddd = "1";
				}
				else if (parseInt(l[1]) > 0 && parseInt(l[2]) < 0) {
					dddd = "2";
					l[2] = Math.abs(parseInt(l[2])).toString();
				}
				else if (parseInt(l[1]) < 0 && parseInt(l[2]) > 0) {
					dddd = "3";
					l[1] = Math.abs(parseInt(l[1])).toString();
				}
				else if (parseInt(l[1]) < 0 && parseInt(l[2]) < 0) {
					dddd = "4";
					l[1] = (Math.abs(parseInt(l[1]))).toString();
				...
				xxxx = l[0].toString() + dddd.toString() + wwww.toString() + l[1] + eeee.toString()  + l[2];
				console.log(i + " : " + xxxx);
				value.t.push(someDate);
				value.o.push(parseFloat(element['시가']));
				value.h.push(parseFloat(element['고가']));
				value.l.push(xxxx);
				value.c.push(parseFloat(element['종가']));
				value.v.push(parseInt(element['거래량']));
			...
			if (value.t.length !== 0) {
				console.log(dateForLogs() + "Successfully parsed and put to cache " + value.t.length + " bars.");
			} else {
				console.log(dateForLogs() + "Parsing returned empty result.");
			}

			var filteredData = filterDataPeriod(value, startDateTimestamp, endDateTimestamp);
			logForData(filteredData, key, false);
			sendResult(JSON.stringify(filteredData));
		}
		else if (result.recordset.length <= 0) {
			sendResult(JSON.stringify(`{ "s": "error" }`, null, 4));
		}
		else {
			sendResult(JSON.stringify(`{ "s": "error" }`, null, 4));
		}
